...
const supabaseUrl =
  process.env.SUPABASE_URL || "https://uthophxqgveapbjvvzqd.supabase.co";
const supabaseKey =
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { imageDataUrl, templateId, mensagem, nome, fotoUrl, imageState } =
      body;

    // Validação básica
    if (!imageDataUrl) {
      return NextResponse.json(
        { success: false, error: "Imagem do cartão é obrigatória" },
        { status: 400 }
      );
    }

    // Gerar um ID único para uso no nome do arquivo (não será inserido no banco, pois o DB gera automaticamente)
    const cardId = uuidv4();
    ...
    const bucketName = buckets?.some((bucket) => bucket.name === "pascoayou")
      ? "pascoayou"
      : "default-bucket";
    ...
    // Removemos o campo "id", pois a coluna "id" da tabela "cards" deve ser gerada automaticamente.
      ...
      .insert([
        {
          ...
          session_id: null, // ou outro valor, se necessário
        },
      ])
      .select();
    ...
    return NextResponse.json({
      success: true,
      cardId, // Retornamos o cardId gerado para uso no front-end (para nome do arquivo, etc.)
      imageUrl,
    ...
    return NextResponse.json(
      {
        success: false,
        error:
          "Falha ao processar a imagem do cartão: " +
